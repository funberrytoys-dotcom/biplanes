...
/** Draw a fallback balloon+chest so the entity is always visible even if the art PNG hasn't loaded. */
function drawFallbackBalloon(g: Graphics) {
  g.clear();
  // rope
  g.moveTo(0, -6).lineTo(0, 30).stroke({ color: 0x4a3a26, width: 2.5, alpha: 0.9 });
  // balloon
  g.ellipse(0, -34, 30, 36).fill({ color: 0x2f7dd6 });
  g.ellipse(0, -34, 30, 36).stroke({ color: 0x1d4f8c, width: 3, alpha: 0.6 });
  g.ellipse(-9, -44, 8, 12).fill({ color: 0x8fc2f2, alpha: 0.55 }); // highlight
